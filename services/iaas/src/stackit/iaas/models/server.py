...
class Server(BaseModel):
    ...
    networking: Optional[ServerNetworking] = None
    ...
    @field_validator("launched_at", mode="before")
    def launched_at_change_year_zero_to_one(cls, value):
        """Workaround which prevents year 0 issue"""
        if isinstance(value, str):
            # Check for year "0000" at the beginning of the string
            # This assumes common date formats like YYYY-MM-DDTHH:MM:SS+00:00 or YYYY-MM-DDTHH:MM:SSZ
        ...
        _obj = cls.model_validate(
            {
                ...
                "name": obj.get("name"),
                "networking": (
                    ServerNetworking.from_dict(obj["networking"]) if obj.get("networking") is not None else None
                ),
                "nics": (
