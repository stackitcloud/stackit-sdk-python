...
class GetCustomRuleGroupResponse(BaseModel):
    """
    Response payload delivering the detailed CRG configuration name and it's rule content.
    """  # noqa: E501

    name: Annotated[str, Field(strict=True)] = Field(description="Custom rule group configuration name.")
    rules: Optional[List[GetCustomRule]] = Field(
        default=None, description="Enriched rules containing auto-generated IDs and computed severity values."
    )
    usage: Optional[CRGUsage] = None
    __properties: ClassVar[List[str]] = ["name", "rules", "usage"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetCustomRuleGroupResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetCustomRuleGroupResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "name": obj.get("name"),
                "rules": (
                    [GetCustomRule.from_dict(_item) for _item in obj["rules"]] if obj.get("rules") is not None else None
                ),
                "usage": CRGUsage.from_dict(obj["usage"]) if obj.get("usage") is not None else None,
            }
        )
        return _obj
