...
class Version(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["createdAt", "disabled", "keyId", "keyRingId", "number", "publicKey", "state"]

    @field_validator("state")
    def state_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["active", "key_material_not_ready", "key_material_invalid", "disabled", "deleting"]):
            raise ValueError(
                "must be one of enum values ('active', 'key_material_not_ready', 'key_material_invalid', 'disabled', 'deleting')"
            )
        return value
