...
class CreateAccessTokenPayload(BaseModel):
    ...
    lifetime: Optional[Annotated[int, Field(le=180, strict=True, ge=1)]] = Field(
        default=None, description="A lifetime period for an access token in days. If unset the token will not expire."
    )
    permissions: List[StrictStr] = Field(description="The access permissions granted to the access token.")
    __properties: ClassVar[List[str]] = ["description", "displayName", "lifetime", "permissions"]
